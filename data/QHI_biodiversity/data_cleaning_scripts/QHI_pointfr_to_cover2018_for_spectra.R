...
# Run for 2018 QHI Data by Gergana Daskalova
...
# Import Data -------------------------------------------------------------------
abundance <- read.csv("scripts/users/gdaskalova/Herschel_ITEXdata_1999-2018_updated.csv", stringsAsFactors = FALSE, strip.white = TRUE)
head(abundance)

# Subsets for the spectral diversity project

#30cm, xy = 5,6
#40cm, xy = 4,5,6,7
#50cm, xy = 4,5,6,7,8
#60cm, xy = 3,4,5,6,7,8
#70cm, xy = 3,4,5,6,7,8,9

abundance <- abundance %>% filter(YEAR == 2018)

abundance <- abundance %>% filter(X %in% c(3, 4, 5, 6, 7, 8, 9) &
                                    Y %in% c(3, 4, 5, 6, 7, 8, 9))
...
QHI_cover2018_ITEX_70 <- cover[,c(7:11,1,12,13,5,14,15,16)]

#Write csv if required-------------------
write.csv(QHI_cover2018_ITEX_70, file="scripts/users/gdaskalova/QHI_cover_2018_70cm.csv")
