...
# for subset of data
(ggplot(subset(full_QHI ,id %in% c(249:259))) +
  ...
  full_QHI <- full_QHI %>%
   mutate(correction_factor = if_else(between(id, 250, 259), "1", reflectance))
 str(HE1_corr)
...
test_2 <- list_of_files %>%
  set_names(.) %>%
  map_df(read.csv2, header = FALSE, sep = "", .id = "FileName") %>%
  mutate(FileName = str_remove_all(FileName, "/Users/shawn/Documents/university work/Dissertation/local/QHI_fieldspec/sort/test_2"),
         id = stringr::str_extract(FileName, "_\\d*"),
         id = str_remove_all(id, "_"),
         id = gsub("(?<![0-9])0+", "", id, perl = TRUE),
...
str(QHI)
names(test_2) <- c("name", "wavelength", "reference", "target", "reflectance", "id", "veg_type", "plot")
...
(p_test_2 <-  ggplot(test_2, aes(x = wavelength, y = reflectance, group = id, color = veg_type)) + 
   geom_line(alpha = 0.3) + 
   theme_spectra() +
   labs(x = "\nWavelength (mm)", y = "Reflectance\n")+ 
...
(p_test_2 <-  ggplot(test_2, aes(x = wavelength, y = reflectance, group = id, color = plot)) + 
    geom_line(alpha = 0.2) + 
    theme_spectra() +
    labs(x = "\nWavelength (mm)", y = "reflectance\n")+ 
    theme(legend.position = "right") +
...
ggplot(test_2_small, aes(x=veg_type, y=spec_mean, fill=veg_type)) + 
...
(p_test_2 <- ggplot(test_2_small, aes(x=veg_type, y=spec_mean, fill=veg_type)) +
...
ggplot(test_2_small, aes(x=veg_type, y=CV, fill=veg_type)) + 
  geom_violin(trim=FALSE, alpha = .5) +
  geom_point(position = position_jitter(0.05)) +
  geom_boxplot(width=0.2, fill="white", alpha = 0.3) +
  theme_cowplot()
...
#plot spectral mean
(p_test_2 <- ggplot(test_2_small_wvlgth, aes(x = wavelength, y = CV, group = plot, color = plot)) + 
